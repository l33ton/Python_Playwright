...
def test_if_navigate_to_my_profile_works(base_url, dashboard_page):
    # Arrange
    # Our arrangement is the dashboard_page fixture
    # Act
    # Dashboard_page fixture is navigating to profile_url
    # Assert
    expect(dashboard_page.page).to_have_url(base_url + PROFILE_URL)
...
@pytest.mark.parametrize("first_name, last_name", [
    ...
    ("O'Neil", "D-r")])
def test_if_changing_name_works(base_url, dashboard_page, first_name, last_name, change_to_original_name):
    # Arrange
...
@pytest.mark.parametrize("new_password", [
    ...
    "Adventuretime1!",
    "FinnTheHuman2+"])
def test_if_changing_password_works_with_valid_data(base_url, dashboard_page, new_password, change_password_to_common):
    # Arrange
